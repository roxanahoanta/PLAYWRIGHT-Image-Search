...
def screenshot():
    ...
    page.wait_for_selector('#W0wltc', timeout=10000)
    page.click('#W0wltc')  # Click the Accept button

    page.wait_for_selector('#APjFqb', timeout=10000)
    page.fill("#APjFqb", SEARCH_TEXT)
    page.press('#APjFqb', "Enter") # Perform search by SEARCH_TEXT

    page.wait_for_selector("#iur", timeout= 10000)
    div = page.query_selector("#iur") # Div contains the images 

    first_image = div.query_selector("img") # Select the first image 
    first_image.screenshot(path="cute_cat_picture.png") # Take a screenshot
